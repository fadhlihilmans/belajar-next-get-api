...
import { Plus, Pencil, Trash2, Search, Loader2, AlertOctagon, X } from "lucide-react"
                  ...
                  filteredData.map((item, index) => (
                    <tr key={item.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <button onClick={() => openEdit(item)} className="text-gray-500 hover:text-gray-900 p-1 mr-2 bg-white border border-gray-200 rounded">
                          <Pencil className="w-4 h-4" />
                        </button>
                        <button onClick={() => openDelete(item.id)} className="text-red-500 hover:text-red-700 p-1 bg-white border border-gray-200 rounded">
